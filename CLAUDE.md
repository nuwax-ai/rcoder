...
- **主要 crate**: `rcoder` (主应用), `agent_runner` (代理运行时), `shared_types` (共享类型), `docker_manager` (容器管理), `rcoder-proxy` (反向代理)
- **Vendor 依赖**: `vendors/symposium-acp/` 包含 SACP 协议实现，支持 Send trait 的 ACP 协议版本
- **排除的 crate**: `tmp/*`, `crates/ai-agents`, `crates/codex-acp-agent` 不参与工作空间编译
...
make test-blocking       # 极端场景测试（包含阻塞）
...
```bash
# 构建 Docker 镜像
make docker-build                    # 构建所有镜像（master-rcoder + rcoder-agent-runner）
make docker-build-master             # 仅构建主服务镜像
make docker-build-agent-runner       # 仅构建 agent-runner 镜像（默认启用 eBPF 调试）
make docker-build-agent-production   # 构建生产镜像（禁用 eBPF 工具，镜像更小）

# 构建基础镜像（很少需要，仅修改系统依赖时）
make docker-build-base               # 构建所有基础镜像
make docker-build-master-base        # 仅构建 master-rcoder-base
make docker-build-agent-base         # 仅构建 rcoder-agent-base
...
### Cargo Features 配置
项目支持多个调试和监控 feature，通过 `CARGO_FEATURES` 环境变量控制：

```bash
# 开发模式（默认，启用所有调试功能）
make docker-build-agent-runner  # 等同于 CARGO_FEATURES="--features ebpf-debug,pyroscope,otel,debug"

# 生产模式（禁用调试工具）
make docker-build-agent-production  # 等同于 CARGO_FEATURES=""

# 自定义 feature 组合
CARGO_FEATURES="--features ebpf-debug,otel" make docker-build-agent-runner
```

**可用的 Features**：
- `ebpf-debug` (docker_manager, rcoder): eBPF 诊断工具，容器以特权模式运行
- `pyroscope` (agent_runner): Pyroscope 性能分析（CPU/Memory）
- `otel` (agent_runner): OpenTelemetry 追踪
- `debug` (rcoder): 调试路由
- `testing` (agent_runner): 测试相关功能
- `test-blocking` (agent_runner): 极端场景阻塞测试

### Pyroscope 性能分析
```bash
# 启动 Pyroscope Server
make pyroscope-up        # Web UI: http://localhost:4040
...
- `crates/shared_types/build.rs` - Proto 编译脚本（使用 tonic-prost-build）
...
**Proto 编译注意事项**：
- 使用 **tonic-prost-build 0.14** 而非 tonic-build（构建依赖）
- 使用 **tonic-prost 0.14** 运行时依赖
- Proto 文件修改后需要 `cargo clean` 并重新编译

### ACP 协议集成
- 使用 **SACP (Symposium ACP) 10.1.0** 替代官方 ACP，核心原因是 SACP 支持 Send trait
- SACP 源代码位于 `vendors/symposium-acp/` 目录
- 依赖 `agent-client-protocol-schema = "0.10.7"` 提供共享类型定义
- AgentSideConnection 和 ClientSideConnection **必须**在 LocalSet 和 spawn_local 中使用
...
2. **禁止编写 unsafe 代码** - 项目要求内存安全（除非有明确原因，如 FFI 调用）
3. **AgentSideConnection 必须在 LocalSet 中使用** - 由于未实现 Send trait
4. **所有响应必须使用中文** - 项目要求统一使用中文响应

### 代码质量要求
基于项目的全局 CLAUDE.md 规则：
- **RwLock 优化**: 并发场景下优先使用 DashMap 替代 `Arc<RwLock<HashMap>>`，注意使用 entry API 避免死锁
- **SOLID 原则**: 开发设计需遵守 SOLID 设计原则
- **Fail Fast**: 异常处理遵守 "Fail Fast" 原则，更早暴露问题
- **禁用 unsafe**: 除非有明确原因（如 FFI 调用），否则不使用 unsafe 代码

### 处理编译警告
遇到 unused 警告时的处理策略：
- `unused_imports`: 移除未使用的导入
- `unused_variables`: 如果变量确实未使用，添加 `_` 前缀（如 `_active_requests`）
- `unused_assignments`: 检查是否为逻辑错误，或移除不必要的赋值
- `dead_code`: 如果代码确实未使用，考虑移除或添加 `#[allow(dead_code)]` 注释说明原因
...
1. **unused_imports**: 直接移除未使用的导入
   ```rust
   // 错误示例
   use tokio::sync::mpsc;  // 未使用

   // 正确：移除该导入
   ```

2. **unused_variables**: 添加下划线前缀表示有意忽略
   ```rust
   // 警告
   let active_requests = ...;
...
3. **unused_assignments**: 检查逻辑是否正确，可能是 bug
   ```rust
   // 警告：多次赋值但未读取
   let mut cancel_success_message = "...";
   cancel_success_message = "...";  // 覆盖前值
   cancel_success_message = "...";  // 再次覆盖

   // 修复：合并为单次赋值或使用条件表达式
   let cancel_success_message = if condition {
       "..."
   } else {
       "..."
   };
...
4. **dead_code**: 未使用的字段或函数
   - 如果确实需要保留（如未来使用或序列化需要），添加 `#[allow(dead_code)]`
   - 否则直接移除
...
```bash
# 1. 清理构建缓存
cargo clean -p shared_types

# 2. 重新编译
cargo build -p shared_types

# 3. 验证生成的代码
ls target/debug/build/shared_types-*/out/
```
...
### gRPC 连接问题
如果 gRPC 连接失败：
1. 检查容器网络配置：`docker network inspect rcoder_agent-network`
2. 检查容器是否运行：`docker ps | grep rcoder-agent`
3. 检查端口映射：gRPC 默认使用 50051 端口
4. 查看容器日志：`docker logs <container_id>`
5. 系统会自动回退到 HTTP 模式（兼容性保障）