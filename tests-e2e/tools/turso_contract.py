"""Execute frozen userApp Turso component contracts; no platform or AI required."""
import hashlib
import json
import os
...
import shutil
import subprocess
from storage_contract_cases import TURSO_TARGETS, passed_exactly_one

REPO = Path(__file__).resolve().parents[2]


def main():
    directory = Path(os.environ['E2E_REPORT_DIR']) / 'turso-contract'
    ...
    try:
        build = subprocess.run(['cargo', 'test', '-p', 'rcoder-storage', '--locked',
                                '--features', 'userapp-turso', '--lib', '--no-run', '--message-format=json'],
                               cwd=REPO, capture_output=True, text=True, timeout=1800)
        (directory / 'build.jsonl').write_text(build.stdout)
        (directory / 'build.stderr').write_text(build.stderr)
        if build.returncode:
            raise RuntimeError('Turso contract compilation failed')
        ...
        (directory / 'identity.json').write_text(json.dumps({
            ...
            'evidence_level': 'turso_component', 'planned_cases': list(TURSO_TARGETS),
        }, indent=2))
        ...
        missing = set(TURSO_TARGETS.values()) - names
        record('Turso frozen cases present', not missing, ', '.join(sorted(missing)))
        if missing:
            return 1
        for case, target in TURSO_TARGETS.items():
            result = subprocess.run([str(frozen), target, '--exact', '--nocapture'],
                                    cwd=REPO, capture_output=True, text=True, timeout=120)
            log = result.stdout + result.stderr
            (directory / (case + '.log')).write_text(log)
            record('Turso ' + case, passed_exactly_one(result.returncode, log))
    except (OSError, ValueError, KeyError, RuntimeError, subprocess.SubprocessError) as error:
        record('Turso contract execution', False, type(error).__name__ + ': ' + str(error))
