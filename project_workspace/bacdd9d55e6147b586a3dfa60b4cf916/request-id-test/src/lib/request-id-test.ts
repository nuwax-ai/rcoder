...
class RequestIdTracker {
  private static instance: RequestIdTracker;
  private SESSION_REQUEST_CONTEXT = new Map<string, string>(); // project_id -> request_id

  static getInstance(): RequestIdTracker {
    if (!RequestIdTracker.instance) {
      RequestIdTracker.instance = new RequestIdTracker();
    }
    return RequestIdTracker.instance;
  }

  /**
   * 模拟 ChatHandler 处理 request_id
   */
  simulateChatHandler(request: ChatRequest): ChatResponse {
   ...
   */
  simulateAcpAgent(chatPrompt: any): { meta: any } {
   ...
   */
  simulateChannelUtils(promptRequest: any, projectId: string): void {
    ...
    if (promptRequest.meta) {
      const reqId = promptRequest.meta.get ?
        promptRequest.meta.get("request_id") :
        promptRequest.meta["request_id"];
    ...
    const startNotify: SessionNotify = {
      type: 'start',
   ...
   */
  simulateSessionNotification(sessionId: string, projectId: string): SessionNotify {
    ...
    // 2. 发送通知
    const notification: SessionNotify = {
      type: 'update',
   ...
   */
  runCompleteFlow(request: ChatRequest): {
    success: boolean;
...
// 导出测试函数
export { RequestIdTracker, runRequestIdTests };
