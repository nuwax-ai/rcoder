#!/bin/bash
# Chrome DevTools MCP (Proxy Mode - chrome-tools) 压力测试脚本
# 用法: ./stress_test_proxy_chrome.sh [并发数] [轮次]
...
echo "🔥 Chrome DevTools MCP (Proxy Mode) 压力测试: ${CONCURRENT} 并发 × ${ROUNDS} 轮"
echo "================================================"
  ...
  for i in $(seq 1 $CONCURRENT); do
    (
      ...
      RESPONSE=$(curl -s -w "\n%{http_code}" --max-time 120 \
        ...
        --data-raw '{
          "user_id": "proxy_chrome_r'$round'_u'$i'",
          "prompt": "ProxyChromeMCP压测'$round'-'$i'",
          "model_provider": {
            "id": "zhipu-glm-4.6",
            "name": "zhipu-glm-4.6",
