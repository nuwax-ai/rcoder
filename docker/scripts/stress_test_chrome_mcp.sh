#!/bin/bash
# Chrome DevTools MCP (Proxy Mode - chrome-devtools) 压力测试脚本
# 用法: ./stress_test_chrome_mcp.sh [并发数] [轮次]
  ...
  for i in $(seq 1 $CONCURRENT); do
    (
      ...
      RESPONSE=$(curl -s -w "\n%{http_code}" --max-time 120 \
        ...
        --data-raw '{
          "user_id": "chrome_mcp_r'$round'_u'$i'",
          "prompt": "ChromeMCP压测'$round'-'$i'",
          "model_provider": {
            "id": "zhipu-glm-4.6",
            "name": "zhipu-glm-4.6",
