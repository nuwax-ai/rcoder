...
/// cache/clean）的 userApp 分派三字段（service_type/app_id/app_stage）必须存在
/// 且有非空 description，且描述写明 app_id 分派定位语义——wire 契约核心
/// （独立 app_id 字段，project_id 回归 computer 语义，见 0db3789）靠此守卫钉住。
...
fn agent_endpoints_userapp_dispatch_fields_are_documented() {
    ...
    // 五 schema × 三字段 = 15（notify-resolved 无 schema 引用不在遍历内）
