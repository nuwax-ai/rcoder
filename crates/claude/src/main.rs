...
use agent::ClaudeAgent;
use agent::ClaudeConfig;
    ...
    local_set.run_until(async move {
        ...
        let config = ClaudeConfig {
            claude_home: PathBuf::from(
                std::env::var("CLAUDE_HOME").unwrap_or_else(|_| "~/.claude".to_string()),
            ),
            cwd: std::env::current_dir()?,
            model: std::env::var("CLAUDE_MODEL").unwrap_or_else(|_| "claude-3-5-sonnet-20241022".to_string()),
        };

        let agent = ClaudeAgent::with_config(tx, client_tx.clone(), config);
