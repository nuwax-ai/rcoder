...
///
/// 推导链（K8s 部署形态，dev builder 复合键 `{user_id}-{app_id}` 时代）：
/// StatefulSet controller 自动给每个 pod 打
/// `apps.kubernetes.io/controller-revision-hash` label，其值 = ControllerRevision
/// 名 = `{sts 名}-{10 位 hash}`。K8s label 值上限 63 字节：
///
/// ```text
/// rcoder-app-builder-(19) + {user_id}-{app_id} + -hash(11) ≤ 63
/// → 复合 identifier ≤ 33（USERAPP_BUILDER_INSTANCE_ID_MAX_LEN）
/// → user_id 现网 10 位 → app_id ≤ 22
/// ```
///
/// identifier 白名单（`IDENTIFIER_RE`）允许 64 字符，但超限的复合 id 在
/// K8s 下创建 builder STS 必然 `FailedCreate: invalid metadata.labels`，
/// 表象是含糊的 ensure 500/连接超时（真因只在 kubectl events）——入口
/// Fail Fast 把它变成明确的 400。Docker 模式理论上限更宽，统一取最紧
/// 约束（与 user_id ≤23 的既有限制同类：K8s 资源名约束传导到业务标识）。
/// 生产 Userapp（`rcoder-app-{app_id}`，非复合）预算更宽松（app_id ≤ 41），
/// 22 同样覆盖。
