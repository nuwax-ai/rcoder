//! Published identity must be usable by control requests from native paths.
...
#[tokio::test]
async fn native_workspace_path_identity_is_accepted_by_stop() {
    let temp = tempfile::tempdir().unwrap();
    let workspace = temp.path().join("中文 project.with spaces");
