//! 系统提示词配置 - 代理模块
//!
//! 此模块已弃用，请使用 `agent_config` 中的 `SystemPromptConfig::get_prompt()` 方法。
//! 保留此模块用于向后兼容。

// 从 agent_config 重新导出
#[allow(deprecated)]
pub use agent_config::PromptBuilder;
pub use agent_config::DEFAULT_SYSTEM_PROMPT;

/// 格式化数据源信息为可读文本
pub fn format_data_sources(data_sources: &[String]) -> String {
...
/// 构建带数据源的用户提示词
pub fn build_user_prompt_with_data_sources(user_prompt: &str, data_sources: &[String]) -> String {
    if data_sources.is_empty() {
...
mod tests {
    ...
    #[test]
    fn test_default_system_prompt_available() {
    ...
    #[test]
    fn test_build_user_prompt_with_data_sources() {
        let user_prompt = "Create a component";
        let data_sources = vec![r#"{"api": "https://api.example.com"}"#.to_string()];
        let result = build_user_prompt_with_data_sources(user_prompt, &data_sources);
        assert!(result.contains(user_prompt));
        assert!(result.contains("<DATA_SOURCES>"));
        assert!(result.contains("api.example.com"));
    }

    #[test]
    fn test_build_user_prompt_without_data_sources() {
        let user_prompt = "Create a component";
