...
pub struct InstallState {
    ...
    lock: Mutex<()>,
    /// 当前正在安装的版本（None = 未在安装）
    installing_version: std::sync::Mutex<Option<String>>,
...
impl InstallState {
    ...
    pub fn set_installing(&self, version: &str) {
        *self.installing_version.lock().unwrap() = Some(version.to_string());
    }

    /// 清除安装状态
    pub fn clear_installing(&self) {
        *self.installing_version.lock().unwrap() = None;
    }

    /// 获取当前正在安装的版本
    pub fn installing_version(&self) -> Option<String> {
        self.installing_version.lock().unwrap().clone()
    }
    ...
    pub fn replace_cancel_token(&self, new_token: CancellationToken) {
        *self.cancel.lock().unwrap() = new_token;
    }
        ...
        self.states
            ...
            .or_insert_with(|| {
                Arc::new(InstallState {
                    lock: Mutex::new(()),
                    installing_version: std::sync::Mutex::new(None),
