...
function overrideBinary() {
  return process.env.FILE_SERVER_BINARY || "";
}
...
async function ensureBinary() {
  ...
  process.stderr.write(
    `Downloading file-server ${VERSION} for ${process.platform}/${process.arch} …\n  ${url}\n`,
  );
  ...
  if (!existsSync(out)) {
    throw new Error(
      `file-server extraction finished but binary not found at ${out}`,
    );
  }
