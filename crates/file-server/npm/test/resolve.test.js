...
function clearOverride() {
  delete process.env.FILE_SERVER_TARGET;
}
clearOverride();
...
test("archive ext and binary name per platform", () => {
  ...
  assert.strictEqual(getBinaryName("linux"), "file-server");
  assert.strictEqual(getBinaryName("win32"), "file-server.exe");
});
...
test("downloadUrl follows OSS path convention", () => {
  ...
  assert.strictEqual(
    url,
    "https://nuwa-packages.oss-rg-china-mainland.aliyuncs.com/file-server/v1.2.3/file-server-1.2.3-x86_64-unknown-linux-gnu.tar.gz",
  );
});

test("FILE_SERVER_TARGET override wins over host detection", () => {
  process.env.FILE_SERVER_TARGET = "aarch64-unknown-linux-gnu";
  assert.strictEqual(
