...
use crate::models::{
    AllFilesBody, AllFilesUpdateResult, SpecifiedBody, SpecifiedFilesUpdateResult,
};
...
#[utoipa::path(post, path = "/specified-files-update", request_body = SpecifiedBody, responses((status = 200, description = "定点更新结果", body = SpecifiedFilesUpdateResult), crate::openapi::ErrorApiResponses), tag = "Code")]
pub(crate) async fn specified_files_update(
    State(state): State<AppState>,
    Json(mut body): Json<SpecifiedBody>,
) -> Result<Json<SpecifiedFilesUpdateResult>, AppError> {
    ...
    .await?;
    Ok(Json(SpecifiedFilesUpdateResult {
        success: true,
        message: "Specified files updated successfully".to_string(),
...
"#,
    responses((status = 200, description = "全量更新结果", body = AllFilesUpdateResult), crate::openapi::ErrorApiResponses), tag = "Code")]
pub(crate) async fn all_files_update(
    State(state): State<AppState>,
    Json(mut body): Json<AllFilesBody>,
) -> Result<Json<AllFilesUpdateResult>, AppError> {
    body.validate().map_err(crate::error::from_garde)?;
    let project_id = body.project_id.trim().to_string();
    ...
    .await?;
    Ok(Json(AllFilesUpdateResult {
        success: true,
        message: "Files submitted successfully".to_string(),
        project_id: result.project_id,
        restarted: false,
    }))
}
