...
implement_store!(
    ...
    "SELECT l.record, d.record, p.record, a.record FROM userapp_lifecycles l LEFT JOIN userapp_operations d ON d.operation_id=json_extract(l.record, '$.active_operations.dev') AND d.app_id=l.app_id LEFT JOIN userapp_operations p ON p.operation_id=json_extract(l.record, '$.active_operations.prod') AND p.app_id=l.app_id LEFT JOIN userapp_operations a ON a.operation_id=json_extract(l.record, '$.active_operations.application') AND a.app_id=l.app_id WHERE l.app_id > $1 ORDER BY l.app_id LIMIT $2"
);
