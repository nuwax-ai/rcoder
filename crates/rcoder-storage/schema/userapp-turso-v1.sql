...
CREATE TABLE userapps (
 ...
 recycle_enabled INTEGER CHECK(recycle_enabled IN (0,1)), wake_on_traffic INTEGER CHECK(wake_on_traffic IN (0,1)),
 idle_timeout_seconds INTEGER CHECK(idle_timeout_seconds>=0),
...
CREATE TABLE userapp_operations (
 ...
 created_at_us INTEGER NOT NULL, updated_at_us INTEGER NOT NULL, terminal_at_us INTEGER,
...
CREATE TABLE userapp_requests (
 ...
 previous_lifecycle_id TEXT, new_lifecycle_id TEXT, created_at_us INTEGER NOT NULL,
...
CREATE TABLE userapp_operation_inputs (
 ...
 payload_digest TEXT NOT NULL CHECK(length(payload_digest)=64), created_at_us INTEGER NOT NULL,
 FOREIGN KEY(operation_id,app_id,lifecycle_id) REFERENCES userapp_operations(operation_id,app_id,lifecycle_id)
);
CREATE TABLE userapp_operation_leases (
 ...
 receipt_version INTEGER NOT NULL CHECK(receipt_version=1), receipt_json TEXT NOT NULL, created_at_us INTEGER NOT NULL,
 FOREIGN KEY(operation_id,app_id,lifecycle_id) REFERENCES userapp_operations(operation_id,app_id,lifecycle_id)
);
...
CREATE TABLE userapp_resource_bindings (
 ...
 created_at_us INTEGER NOT NULL, PRIMARY KEY(service_type,physical_uid),
...
CREATE TABLE userapp_activity (
 ...
 last_accessed_at_us INTEGER NOT NULL, updated_at_us INTEGER NOT NULL,
...
CREATE TABLE userapp_runtime_config_versions (
 ...
 scope TEXT NOT NULL CHECK(scope IN ('dev','prod')), version INTEGER NOT NULL CHECK(version>=1),
 request_id TEXT NOT NULL CHECK(length(request_id)>0), expected_revision INTEGER NOT NULL CHECK(expected_revision>=0),
 payload_version INTEGER NOT NULL CHECK(payload_version=1),
...
CREATE TABLE userapp_runtime_configs (
 app_id TEXT NOT NULL, lifecycle_id TEXT NOT NULL, scope TEXT NOT NULL CHECK(scope IN ('dev','prod')),
 revision INTEGER NOT NULL CHECK(revision>=1), saved_version INTEGER NOT NULL,
 applied_version INTEGER, applying_version INTEGER, applying_operation_id TEXT,
 updated_at_us INTEGER NOT NULL, PRIMARY KEY(app_id,lifecycle_id,scope),
...
CREATE TABLE userapp_operation_configs (
 operation_id TEXT NOT NULL PRIMARY KEY, app_id TEXT NOT NULL, lifecycle_id TEXT NOT NULL,
 scope TEXT NOT NULL CHECK(scope IN ('dev','prod')), config_version INTEGER NOT NULL,
