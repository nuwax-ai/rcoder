...
- **Hotpath 本地剖析**（仅本地开发，默认关闭 cargo feature）：本地 `cargo run -p rcoder --features hotpath`（叠加 `hotpath-alloc` 内存 / `hotpath-mcp` AI 查询，agent_runner 同名 feature）；Ctrl+C 或正常退出自动打报告；TUI 先 `cargo install hotpath --features=tui` 再 `hotpath console` 连 127.0.0.1:6770；MCP 接入 `claude mcp add --transport http hotpath http://localhost:6771/mcp`。Docker dev 链路（dev-up/dev-build 默认 features、dev-hot 热编译）已恒编入 `hotpath`：`make dev-hot` 的 `docker restart` 触发 SIGTERM → graceful shutdown 报告落容器 stdout（`docker logs` 可见）；容器内 6770/6771 绑死 127.0.0.1（端口映射无效），实时数据 `docker exec <容器> curl -s 127.0.0.1:6770/functions` 等 JSON 端点（与 MCP 工具同源）。常用 env：`HOTPATH_SHUTDOWN_MS=N`（定时出报告退出）、`HOTPATH_OUTPUT_FORMAT=json|none`、`HOTPATH_OUTPUT_PATH`；进程同机并用时以 `HOTPATH_METRICS_PORT`/`HOTPATH_MCP_PORT` 错开默认 6770/6771（撞口优雅降级不 panic）。生产构建（build-agent-docker）不含该 feature，零影响。
