...
| P1-01 强化 | legacy main startup order fix: bind before deploy_stage + remove LivenessHold | cargo nextest run --manifest-path crates/app-cli/Cargo.toml --all-features（186 通过，含新测试 legacy_deploy_url_with_port_conflict_has_no_deploy_side_effects） | 0 | API bind 在 deploy_stage 前 fail-fast；APP_DEPLOY_URL + 端口冲突无部署副作用（.incoming/.staging/.deploy-state.toml 不存在）；LivenessHold 退役；clippy 零 warning | targeted tests | — | Compose / remote-k8s / 全量 workspace 门禁 |
